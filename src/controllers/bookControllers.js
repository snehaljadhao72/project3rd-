...
const getBooks = async function (req, res) {
    try {
        ...
        if(data.userId){
            if(!mongoose.Types.ObjectId.isValid(data.userId)) if(!mongoose.Types.ObjectId.isValid(bookId)) return res.status(400).send({status:false, message:"invalid BookId"})
        }
        if(data.category){
